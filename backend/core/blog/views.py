...
from .editorial import estimate_word_count
...
def search_posts(request):
    ...
    if use_postgres_search:
        ...
        fallback_filter = (
            ...
            | Q(search_headings__icontains=query)
...
def editorial_autosave(request):
    if request.method == "GET":
        ...
        autosave = (
            EditorialAutosave.objects.select_related("post")
            .filter(author=request.user, draft_key=draft_key)
            .first()
        )
        if not autosave:
            return Response(
    ...
    draft_key = (
        str(serializer_input["draft_key"] or "").strip()[:80]
        or EDITORIAL_AUTOSAVE_DEFAULT_KEY
    )
    ...
    logger.info(
        "editorial_autosave_saved author_id=%s draft_key=%s word_count=%s",
        request.user.id,
        draft_key,
        autosave.word_count,
    )
